...
public partial class StoredProcedures
{
	...
	public static void CLR_GetEasyForexReport(DateTime ToDay)
	{
		...
		if (DateTime.Now.Day == 1)
			fromDate = string.Format("{0}{1}{2}", ToDay.Year, (ToDay.Month-1).ToString().Length == 1 ? "0" + (ToDay.Month - 1).ToString() : (ToDay.Month -1).ToString(), "01");
		else
			fromDate = string.Format("{0}{1}{2}", ToDay.Year, ToDay.Month.ToString().Length == 1 ? "0" + ToDay.Month.ToString() : ToDay.Month.ToString(), "01");
