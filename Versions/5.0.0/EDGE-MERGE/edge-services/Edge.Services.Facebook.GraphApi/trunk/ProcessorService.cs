...
namespace Edge.Services.Facebook.GraphApi
{
    ...
    public class ProcessorService : MetricsProcessorServiceBase
    {
        ...
        static class MeasureNames
        {
            ...
            public const string ConversionManyPerClickFieldName = "TotalConversionsOnePerClick";
                                    ...
                                    if (actions.Count > 0)
                                    {
                                        ...
                                        List<Dictionary<string, object>> action_items = action_list.Where(dict => dict.ContainsKey("action_type") && dict.ContainsKey("value") &&
                                            (
                                                ...
                                                dict["action_type"].ToString().Equals("link_click") ||
                            ...
                            if (adGroupStatsReader.Current.adgroup_id != null)
                            {
                                ...
                                if (ads.TryGetValue(adGroupStatsReader.Current.adgroup_id, out tempAd))
                                {
                                    ...
                                    if (conversion_data.TryGetValue(adGroupStatsReader.Current.adgroup_id,out adgroup_conversion_data))
                                    {
                                        ...
                                        var ConversionsManyPerClick = from element in adgroup_conversion_data where element["action_type"].Equals("offsite_conversion") select element["value"];
                                        ...
                                        var link_click = from element in adgroup_conversion_data where element["action_type"].Equals("link_click") select element["value"];
                                        ...
                                        adMetricsUnit.MeasureValues.Add(this.ImportManager.Measures[MeasureNames.ConversionManyPerClickFieldName], Convert.ToDouble(ConversionsManyPerClick.DefaultIfEmpty("0").FirstOrDefault()));
                                        ...
                                        /* Sets totals clicks for validations */
                                        if (currentOutput.Checksum.ContainsKey(Measure.Common.Clicks))
                                            currentOutput.Checksum[Measure.Common.Clicks] += Convert.ToDouble(link_click.DefaultIfEmpty("0").FirstOrDefault());
                                      
                                        /* Sets measures values */

                                        adMetricsUnit.MeasureValues.Add(this.ImportManager.Measures[Measure.Common.Clicks], Convert.ToDouble(link_click.DefaultIfEmpty("0").FirstOrDefault()));
                                    }
