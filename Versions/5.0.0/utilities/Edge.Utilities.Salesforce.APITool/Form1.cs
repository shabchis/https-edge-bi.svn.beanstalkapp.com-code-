...
namespace Edge.Utilities.Salesforce.APITool
{
    public partial class Form1 : Form
    {
        ...
        private void test_query_Click(object sender, EventArgs e)
        {

            
            //Regex for Calendar units.
            MatchCollection calendarMatches = Regex.Matches(this.soql.Text, @"TimePeriod.EqualToCalendarUnits\(([A-Z\.a-z_]+)\)", RegexOptions.IgnoreCase);
            if (calendarMatches.Count > 0)
            {
                foreach (Match calendarMatch in calendarMatches)
                {
                    ...
                    this.soql.Text = this.soql.Text.Replace(string.Format("TimePeriod.EqualToCalendarUnits({0})", dataParamName), string.Format(" CALENDAR_YEAR({0})={1} AND CALENDAR_MONTH({0})={2} AND DAY_IN_MONTH({0}) = {3} ", dataParamName, DateTime.Now.AddDays(-1).Year, DateTime.Now.AddDays(-1).Month, DateTime.Now.AddDays(-1).Day));
                }
            }

            //Regex for TimePeriodStringFormat units.
            MatchCollection timeMatches = Regex.Matches(this.soql.Text, @"TimePeriod.EqualToString\(([A-Z\.a-z_]+)\)", RegexOptions.IgnoreCase);
            if (timeMatches.Count > 0)
            {
                foreach (Match calendarMatch in timeMatches)
                {
                    string dataParamName = calendarMatch.Groups[1].Value;
                    string sfTimePeriodStartFormat = string.Format("{0}T00:00:00.00Z", DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd"));
                    string sfTimePeriodEndFormat = string.Format("{0}T23:59:59.59Z", DateTime.Now.AddDays(-1).ToString("yyyy-MM-dd"));
                    this.soql.Text = this.soql.Text.Replace(string.Format("TimePeriod.EqualToString({0})", dataParamName), string.Format("{0}>{1} AND {0}<{2} ", dataParamName, sfTimePeriodStartFormat, sfTimePeriodEndFormat));
                }
            }
